...
import { ArrowLeft, Clock, Tag, Building2, ExternalLink } from 'lucide-react';
                ...
                <Link to={`/news/${article.id}`} key={article.id}
                  ...
                  <div className="flex items-center gap-2 mb-3 flex-wrap">
                    <span className="text-xs bg-green-100 text-green-700 px-2.5 py-1 rounded-full font-semibold">
                      {ARTICLE_TYPE_LABELS[article.articleType] || 'ブログ'}
                    </span>
                  ...
                  <div className="mt-4 text-xs text-green-700 font-semibold group-hover:translate-x-1 transition-transform inline-flex items-center gap-1">
                    続きを読む →
                  </div>
                </Link>
              );
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        <div className="flex items-center gap-3 mb-8 bg-[#f5f0e8] rounded-xl px-4 py-3">
          ...
          <div>
            <p className="text-sm font-bold text-[#3d2e1e]">田中（スタッフ）</p>
            <p className="text-xs text-[#999]">長門不動産・長門市在住10年</p>
          </div>
        </div>
            ...
            <div className="grid gap-3">
              {relatedProps.map(p => (
                <Link key={p.id} to={`/property/${p.id}`}
                  className="flex items-center gap-3 border border-[#e8e0d4] rounded-xl p-3 hover:border-green-300 hover:shadow-sm transition-all group">
                  {p.images?.[0]
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs font-bold text-green-700 mt-0.5">
                      {p.type === 'rent' ? `¥${(p.rent || p.price).toLocaleString()}/月` : `¥${p.price.toLocaleString()}`}
                    </p>
                  </div>
        ...
        <div className="mt-12 bg-green-50 border border-green-200 rounded-2xl p-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <a href="tel:0837-22-3321"
              className="inline-flex items-center justify-center gap-2 bg-green-700 text-white px-6 py-2.5 rounded-lg hover:bg-green-800 font-bold text-sm transition-colors">
              📞 0837-22-3321
            </a>
            <Link to="/contact"
              className="inline-flex items-center justify-center gap-2 border-2 border-green-700 text-green-700 px-6 py-2.5 rounded-lg hover:bg-green-50 font-bold text-sm transition-colors">
              メールで問い合わせる
            </Link>
          </div>
        </div>
